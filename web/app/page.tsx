              ...
              <div className="flex gap-3">
                <button
                  onClick={async () => {
                    ...
                    await navigator.clipboard.writeText(dailyMessage.content)
                    const { createClient } = await import('@/lib/supabase/client')
                    const supabase = createClient()
                    ...
                    if (user && profile) {
                      await supabase.from('history').insert({
                        ...
                        beneficiary_name: 'Nespecificat',
                      })
                      const today = new Date().toISOString().split('T')[0]
                      const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString().split('T')[0]
                      const { data: freshProfile } = await supabase.from('profiles').select('total_xp, streak_count, last_active_date').eq('id', user.id).single()
                      let newStreak = 1
                      if (freshProfile) {
